...
public partial class ShareConnectionControl : ContentView
{
    private ConnectionInfo _connectionInfo = null!;
    private string _generatedPassword = string.Empty;

    public event EventHandler<ShareConnectionEventArgs>? ShareCompleted;
    public event EventHandler? Cancelled;
    public event EventHandler<string>? ErrorOccurred;

    public ShareConnectionControl()
    {
        InitializeComponent();
        SetupEventHandlers();
    }
    ...
    private void GeneratePassword()
    {
        ...
        {
            ErrorOccurred?.Invoke(this, $"Không thể tạo mật khẩu: {ex.Message}");
        }
    }
    private async void CopyPasswordBtn_Clicked(object sender, EventArgs e)
    {
        ...
        {
            ErrorOccurred?.Invoke(this, $"Không thể copy mật khẩu: {ex.Message}");
        }
    }
        ...
        try
        {
            ...
            if (EncryptCheckBox.IsChecked)
            {
                ...
                else
                {
                    ...
                    {
                        ErrorOccurred?.Invoke(this, "Vui lòng nhập mật khẩu!");
                        return;
                    }
                }
            }
            ...
            if (!success)
            {
                ErrorOccurred?.Invoke(this, $"Không thể tạo tệp: {filePath}");
                return;
            }
            ...
            // Tự động mở thư mục nếu được chọn
            if (AutoOpenFolderCheckBox.IsChecked)
            {
                await FileHelper.OpenFileLocation(filePath);
            }
        }
        catch (Exception ex)
        {
            ErrorOccurred?.Invoke(this, $"Có lỗi xảy ra: {ex.Message}");
        }
        finally
        {
    ...
    private void ShowResult(string filePath, bool isEncrypted)
    {
        ...
        // Gọi event
        ShareCompleted?.Invoke(this, new ShareConnectionEventArgs(filePath, isEncrypted));
    }
    private async void OpenLocationBtn_Clicked(object sender, EventArgs e)
    {
        ...
        {
            ErrorOccurred?.Invoke(this, $"Không thể mở thư mục: {ex.Message}");
        }
    }

    private void CloseBtn_Clicked(object sender, EventArgs e)
    {
        ShareCompleted?.Invoke(this, new ShareConnectionEventArgs(FilePathLabel.Text, false));
    }
...
public class ShareConnectionEventArgs : EventArgs
{
    public string FilePath { get; }
    public bool IsEncrypted { get; }

    public ShareConnectionEventArgs(string filePath, bool isEncrypted)
    {
        FilePath = filePath;
